...
local function get_search_pattern(pattern)
    ...
    if #format_parts == 2 then
        ---@diagnostic disable-next-line
        return sanitize_for_pattern(
            vim.fn.printf('\\(?:%s\\)\\?%s\\(?:%s\\)\\?', format_parts[1], pattern, format_parts[2])
        )
    end
end
...
local function replace_line_with_buf(lnum, buf)
    ...
    if is_comment(lnum, curbuf) then
        local formatstring = sanitize_for_pattern(vim.fn.printf(vim.bo.commentstring, '\\1'))
...
replace_case['string'] = function(ctx, rhs)
    local subst_command = '%s/' .. sanitize_for_pattern(ctx.pattern) .. '/' .. sanitize_for_pattern(rhs) .. '/g'
...
replace_case['system'] = function(ctx, rhs)
    ...
    vim.print("command '" .. table.concat(rhs, ' ') .. "' ended in: " .. result.stderr)
end
